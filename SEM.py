...
# MASKING FIBER AREA BERDASARKAN EDGE (HALUS)
# =========================
# Dilate edge mask to cover full fiber width, lalu haluskan
kernel = np.ones((3,3), np.uint8)  # Lebih kecil untuk hasil lebih halus
fiber_area_mask = cv2.dilate(edges_canny_sharp, kernel, iterations=1)
fiber_area_mask = cv2.morphologyEx(fiber_area_mask, cv2.MORPH_CLOSE, kernel)
fiber_area_mask = cv2.GaussianBlur(fiber_area_mask, (5,5), 0)  # Blur untuk haluskan tepi
fiber_area_mask = fiber_area_mask > 10  # Threshold agar tetap biner

masked_fiber = img.copy()
masked_fiber[~fiber_area_mask] = 0
...
plt.subplot(1,2,2)
plt.imshow(cv2.cvtColor(masked_fiber, cv2.COLOR_BGR2RGB))
plt.title("Masked Fiber Area (Dilated Edge)")
plt.axis('off')
